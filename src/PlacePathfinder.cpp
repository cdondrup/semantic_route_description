...
std::vector<std::vector<std::string>> PlacePathfinder::find(std::string& my_place, std::string& goal_place, std::string& region)
{
  ...
  std::vector<std::string> to_corridors = onto_->string2vector(onto_->getOn(goal_place, "isAlong"));
...
void PlacePathfinder::removeNotInRegion(std::vector<std::string>& corridors, std::string& region)
{
  ...
  {
    std::vector<std::string> regions = onto_->string2vector(onto_->getOn(corridors[i], "isIn"));
